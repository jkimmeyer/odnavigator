class CityPortal < ApplicationRecord
  ...
  def package_list
    request_url = self.data_portal.url + '/package_list'
    ...
    begin
      ...
      if response.code < 300
         single_dataset_creation(nested_hash_value(JSON.parse(response.body),'result'))
      else
        puts "Server returns a #{response.code} Status Code."
      end
    end
...
#           # TODO simplify
#           datasets = nested_hash_value(JSON.parse(response.body), 'result')
...
def single_dataset_creation(result)
  result.each do |f|
    Dataset.create(title: f)
  end
end
...
def recursive_dataset_creation(result)
  ...
  result.each do |package|
    ...
    if package.is_a? Hash then
      puts package.fetch('resources', nil)
